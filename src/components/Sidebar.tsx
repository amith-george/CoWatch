      ...
      <aside className="hidden sm:flex fixed top-0 left-0 h-screen w-24 bg-[#1f1f1f] text-white flex-col justify-between items-center py-6 shadow-lg">
        ...
        <div className="flex flex-col items-center space-y-10 font-sans text-sm">
          <div className="flex flex-col items-center space-y-6 mb-4">
            {sidebarItems.slice(0, 4).map(({ label, icon: Icon, onClick }) => (
              <button
                key={label}
                onClick={onClick}
          ...
          <button
            onClick={sidebarItems[4].onClick}
            className="flex flex-col items-center text-white group"
          >
            <div className={ICON_BOX_CLASSES}>
              <UserIcon className="w-8 h-8" />
            </div>
            <p className="mt-1 text-sm font-medium text-center">Random</p>
          </button>
        </div>
      </aside>

      {/* Mobile Bottom Bar */}
      <aside className="sm:hidden fixed bottom-0 left-0 w-full bg-[#1f1f1f] text-white flex justify-around items-center px-2 py-3 shadow-lg">
